...
public class PerformanceDAO {
	...
	public List<PerformanceVO> selectPerformanceList(String pNo) throws Exception {
		...
		try {
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select po.SYSTEM_FILE_NAME, perf.title, to_char(perf.start_Date,'YYYY/MM/DD'), to_char(perf.end_Date,'YYYY/MM/DD'),	");
			sql.append("to_char(sch.s_date, 'YYYY/MM/DD'), to_char(o.o_time,'HH24:MI'), perf.price											");
			sql.append("from poster po, performance perf																					");
			sql.append("where perf.p_no = po.p_no																							");
			sql.append("and perf.p_no = sch.p_no																							");
			sql.append("and sch.s_no = o.s_no																								");
			sql.append("and po.main_poster = '1'																							");
			sql.append("and perf.p_no = ?																									");
